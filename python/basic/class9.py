...
response = requests.get('https://api.vc.bilibili.com/link_draw/v1/doc/doc_list?uid=6823116&page_num=0&page_size=30&biz=all',headers=headers)
obj =  json.loads(response.text)
imgArray = obj.get('data').get('items')
for aimg in imgArray:
    imgURL = aimg.get('pictures')[0].get('img_src')
    img = requests.get(imgURL).content
    filename = str(aimg.get('doc_id'))+'.jpg'
    print('文件名:',filename)
